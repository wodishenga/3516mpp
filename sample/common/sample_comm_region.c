...
HI_S32 SAMPLE_REGION_CreateOverLayEx(HI_S32 HandleNum)
{
    ...
    stRegion.enType = OVERLAYEX_RGN;        //区域类型
    stRegion.unAttr.stOverlayEx.enPixelFmt = PIXEL_FORMAT_ARGB_1555;    //像素格式
    stRegion.unAttr.stOverlayEx.stSize.u32Height = 972;                 //区域大小
    stRegion.unAttr.stOverlayEx.stSize.u32Width  = 1728;
    stRegion.unAttr.stOverlayEx.u32BgColor = 0x00ff00ff;                //背景颜色
    stRegion.unAttr.stOverlayEx.u32CanvasNum =2;                        //区域的内存数量                   
...
HI_S32 SAMPLE_COMM_REGION_AttachToChn(HI_S32 HandleNum,RGN_TYPE_E enType, MPP_CHN_S *pstMppChn)
{
    ...
    switch(enType)
        ...
        case OVERLAYEX_RGN:
            ...
            stChnAttr.unChnAttr.stOverlayExChn.u32BgAlpha = 0;   //Alpha 位为 0 的像素点的透明度,[0,255],取值越小，越透明，0表示全透明
            stChnAttr.unChnAttr.stOverlayExChn.u32FgAlpha = 128;   //Alpha 位为 1 的像素点的透明度,[0,255],取值越小，越透明
            break;
        case COVER_RGN:
            MinHadle = CoverMinHandle;
    ...
    for(i =MinHadle; i <MinHadle+HandleNum;i++)
    {
        ...
        {
            stChnAttr.unChnAttr.stOverlayExChn.stPoint.s32X = 20 + 200*(i - OverlayExMinHandle);   //区域位置
            stChnAttr.unChnAttr.stOverlayExChn.stPoint.s32Y = 20 + 200*(i - OverlayExMinHandle);
            stChnAttr.unChnAttr.stOverlayExChn.u32Layer = i - OverlayExMinHandle;                  //区域层次
        }
        if(COVER_RGN == enType)
        {
...
HI_S32 SAMPLE_COMM_REGION_GetUpCanvas(RGN_HANDLE Handle)
{
	...
	//设置各个区域位图信息，获取区域的显示画布信息主要用于 overlay 或 overlayex 类型导入位图数据
	/*本接口用于获取区域对应的画布信息，在得到画布地址之后，用户可直接对画布
	进行操作，譬如：将 bmp 数据直接填写到该画布中。然后通过调用
	HI_MPI_RGN_UpdateCanvas 接口，更新显示画布数据*/
    ...
    stBitmap.pData   = (HI_VOID*)(HI_UL)stCanvasInfo.u64VirtAddr;   //画布的虚拟地址
    stSize.u32Width  = stCanvasInfo.stSize.u32Width;                //画布的大小
    stSize.u32Height = stCanvasInfo.stSize.u32Height;
	SAMPLE_PRT("stCanvasInfo.stSize.u32Width = %d,  stCanvasInfo.stSize.u32Width = %d!\n", stSize.u32Width,stSize.u32Height);
