...
SAMPLE_Noise_Reduction(VI_PIPE SnapPipe)
{
	...
	stNRAttr.stManual.au8ChromaStr[0] = 2;
	stNRAttr.stManual.au8ChromaStr[1] = 2;
	stNRAttr.stManual.au8ChromaStr[2] = 2;
	stNRAttr.stManual.au8ChromaStr[3] = 2;
	
	stNRAttr.stManual.u8FineStr = 0x50;
	stNRAttr.bNrLscEnable = HI_FALSE;

	stNRAttr.stManual.u16CoringWgt = 0x90;

	stNRAttr.stManual.au16CoarseStr[0] = 0x260;
	stNRAttr.stManual.au16CoarseStr[1] = 0x260;
	stNRAttr.stManual.au16CoarseStr[2] = 0x260;
	stNRAttr.stManual.au16CoarseStr[3] = 0x260;
...
HI_S32 SAMPLE_COMM_VI_StartIsp(SAMPLE_VI_INFO_S* pstViInfo)
{
    ...
    for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
    {
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
			...
			if(ViPipe == 0)
			{
					SAMPLE_Noise_Reduction(ViPipe);
					if (s32Ret != HI_SUCCESS)
            		{
               	 		SAMPLE_PRT("SAMPLE_Noise_Reduction with %#x!\n", s32Ret);
                		SAMPLE_COMM_ISP_Stop(ViPipe);
                		return HI_FAILURE;
            		}
			}
