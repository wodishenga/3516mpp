...
HI_VOID 
SAMPLE_Yuv420_to_Mat(Mat &dst, unsigned char* pYUV420, int width, int height, int Flag)
{
	if (!pYUV420) {
		return;
	}
	...
	if(Flag == 0) {
		...
		printf("save begin.jpg image success \n");
	} else {
		imwrite("end.jpg", dst);
		printf("save end.jpg image success \n");
	}
...
/*保存图片*/
HI_S32 
SAMPLE_SVP_NNIE_Yolov3_SaveBitMap(VIDEO_FRAME_S* pVBuf, int Flag)
{
	HI_S32 s32Ret = HI_SUCCESS;
	unsigned int w, h;
	char* pVBufVirt_Y;
	...
	SAMPLE_Yuv420_to_Mat(m, pBuf, WIDTH, HEIGHT,Flag);
