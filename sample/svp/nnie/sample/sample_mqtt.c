...
char serverIP[50] = "192.168.1.234:18884";
//char serverIP[50] = "192.168.1.69:1883";

SERVER_CONFIG serverConfig = {0};
...
static int 
isSendtoMe(char *topicName)
{
	int len = strlen(subTopic);
	for (int i = 0; i < len; i++) {
			if(topicName[i] != subTopic[i])
			return 0;
	}
	return 1;
}
...
void parseServerConfig(char * text, SERVER_CONFIG *pServerConfig)
{
	cJSON * root = NULL;
	cJSON * item = NULL;
	cJSON * array = NULL;
    ...
    if (!root) {
		...
		item = cJSON_GetObjectItem(root, "msg_content");
		if (item) {
			item = cJSON_GetObjectItem(item, "camera_conf");
			if(item) {
				item = cJSON_GetObjectItem(item, "cameras");
				if (item) {
					array = cJSON_GetArrayItem(item, 0);
					if(array) {
						item = cJSON_GetObjectItem(array, "act");
						if(item) {
							printf("%s:", item->string);   
				        	printf("%s\n", item->valuestring);
						}

						item = cJSON_GetObjectItem(array, "allow_play");
						if(item) {
							pServerConfig->allow_play = item->valueint;
				        	printf("%s:", item->string);   
				       		printf("%d\n", item->valueint);
						}


						item = cJSON_GetObjectItem(array, "attireLocs");
						if(item) {
							strcpy(pServerConfig->attireLocs, item->valuestring);
				        	printf("%s:", item->string);   
				        	printf("%s\n", item->valuestring);
						}


						item = cJSON_GetObjectItem(array, "brand");
						if(item) {
							printf("%s:", item->string);   
				        	printf("%s\n", item->valuestring);
						}
						
						item = cJSON_GetObjectItem(array, "channel");
						if(item) {
							printf("%s:", item->string);   
							printf("%d\n", item->valueint);
						}
						...
						item = cJSON_GetObjectItem(array, "httpPort");
						if(item) {
							printf("%s:", item->string);   
							printf("%d\n", item->valueint);
						}

						item = cJSON_GetObjectItem(array, "ip");
						if(item) {
							printf("%s:", item->string);   
							printf("%s\n", item->valuestring);
						}

						item = cJSON_GetObjectItem(array, "mac");
						if(item) {
							printf("%s:", item->string);   
				        	printf("%s\n", item->valuestring);
						}
						...
						item = cJSON_GetObjectItem(array, "model");
						if(item) {
							pServerConfig->model = item->valueint;
							printf("%s:", item->string);   
							printf("%d\n", item->valueint);
						}


						item = cJSON_GetObjectItem(array, "motionless_time");
						if(item) {
							pServerConfig->motionless_time = item->valueint;
							printf("%s:", item->string);  
							printf("%d\n", item->valueint);
						}

						
						item = cJSON_GetObjectItem(array, "moving_range");
						if(item) {
							pServerConfig->moving_range = item->valueint;
							printf("%s:", item->string);  
							printf("%d\n", item->valueint);
						}

						item = cJSON_GetObjectItem(array, "parkingTime");
						if(item) {
							pServerConfig->parkingTime = item->valueint;
							printf("%s:", item->string);   
							printf("%d\n", item->valueint);
						}


						item = cJSON_GetObjectItem(array, "pwd");
						if(item) {
							printf("%s:", item->string);   
							printf("%s\n", item->valuestring);
						}

						item = cJSON_GetObjectItem(array, "to");
						if(item) {
							char to[40];
							strcpy(to, item->valuestring);
							pServerConfig->to= atoi(to);
						
							printf("pServerConfig->to = %d\n", pServerConfig->to);
...
msgarrvd(void *context, char *topicName, int topicLen, MQTTClient_message *message)
{
    char* payloadptr;
	char text[500] = {0};
	memset(&serverConfig, 0 ,sizeof(SERVER_CONFIG));
    printf("     topic: %s\n", topicName);

	if(isSendtoMe(topicName)) {
		payloadptr = (char *)message->payload;
		memcpy(text, payloadptr, message->payloadlen);
		...
		printf("%s\n", text);
		parseServerConfig(text, &serverConfig);	
	}
...
mqtt_subscribing_process(void)
{
	...
	//请求服务器配置
	pubInfo.pub_id  = PUB_GetServerConfig;
